...
from vision_transformer.vit_model import VisionTransformer, ConcatClassTokenAddPosEmbed, Block, PatchEmbed
...
def deconv_module(hidden_state, num_filters=64, num_outputs=3, kernel_size=3,
    ...
    end_points = {}
    spatial_pad_3 = np.array([[0, 0], [3, 3], [3, 3], [0, 0]])
    with tf_layers.arg_scope(cyclegan_arg_scope()):
        with tf.variable_scope('decoder'):
            with tf_layers.arg_scope(
                    [tf_layers.conv2d],
                    kernel_size=kernel_size,
                    ...
                    activation_fn=tf.nn.relu):
                with tf.variable_scope('decoder1'):
                    net = upsample_fn(hidden_state, num_outputs=num_filters * 2, stride=[4, 4])
                    end_points['decoder1'] = net

                with tf.variable_scope('decoder2'):
                    net = upsample_fn(net, num_outputs=num_filters, stride=[4, 4])
                    end_points['decoder2'] = net
...
def cyclegan_convlstm(inputs, num_filters=64,
    ...
    with tf_layers.arg_scope(cyclegan_arg_scope()):
        ...
        #         # 8 56 56 768
        #         net = tf_layers.conv2d(net, num_filters * 12)
        #         # 8 56*56 768
        #         inputs = tf.reshape(net, [batch_size * time_steps, -1, num_filters * 12])
        ...
        patch_embed = PatchEmbed(img_size=img_size, patch_size=patch_size, embed_dim=embed_dim)
        x = patch_embed(inputs)

        cls_token_pos_embed = ConcatClassTokenAddPosEmbed(embed_dim=embed_dim,
                                                          num_patches=num_patches,
                                                          name="cls_pos")
        x = cls_token_pos_embed(x)
        pos_drop = layers.Dropout(drop_ratio)
        x = pos_drop(x)
        blocks = [Block(dim=embed_dim, num_heads=num_heads, qkv_bias=qkv_bias,
        ...
        x = x[:, :196, :]
        # 2 4 56 56 768
        x = tf.reshape(x, [batch_size, time_steps, 14, 14, embed_dim])
        # 2 56 56 768
        hidden_state = x[:, -1, :, :, :]
        ...
        end_points['hidden_state'] = hidden_state
    # outputs 2 224 224 3 hidden_state 2 56 56 256
    # outputs 2 56 56 3 hidden_state 2 14 14 768
    ...
    with tf.contrib.framework.arg_scope(cyclegan_arg_scope()):
        ...
        if use_decoder:
            for i, layer in enumerate(range(num_down_samples - 2, -1, -1)):
                ...
                h_deconv = tf_layers.conv2d_transpose(inputs=in_node, num_outputs=features // 2, kernel_size=2,
                                                      stride=2)
                h_deconv_concat = tf.concat([conv[layer], h_deconv], axis=3)
...
if __name__ == '__main__':
    input_tensor = tf.placeholder(shape=[2, 4, 224, 224, 3], dtype=tf.float32)
    ...
    outputs, hidden_state, end_points = cyclegan_convlstm(inputs=input_tensor, num_outputs=3, num_filters=64, use_decoder=True)
