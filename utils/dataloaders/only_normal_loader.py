...
class NormalDataLoader(BaseDataAbstractLoader):
    ...
    def get_video_clip(self, video, start, end, interval=1):
        # assets video_name in self._videos_info, 'video {} is not in {}!'.format(video_name, self._videos_info.keys())
        # assets (start >= 0) and (start <= end) and (end < self._videos_info[video_name]['length'])
